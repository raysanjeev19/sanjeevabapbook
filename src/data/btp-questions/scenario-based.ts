...
export const scenarioBasedQuestions: BtpQuestion[] = [
  ...
  },
  {
    ...
    estimatedMinutes: 3,
    expectedAnswer:
      "Check the deployment logs for exactly which module/resource failed and why; MTA deployment provides no cross-module atomicity or transactional rollback guarantee, so a partial failure can genuinely leave the landscape in a mixed-version state (e.g. an app module still running against a database schema it wasn't built for) — you must explicitly verify each affected module's actual running state and version rather than assume anything, then fix the root cause and re-run the deployment.",
    detailedAnswer:
      "First, get the specific failure reason from the deployment logs — a failed resource provisioning (e.g. HANA quota exceeded), a module that failed to start (bad config, missing env var), or a timeout are all different root causes needing different fixes. Critically, the MTA deployment process does NOT guarantee atomicity across modules — there's no true transactional rollback spanning the whole MTA, so a failure partway through can genuinely leave your landscape in a mixed-version state: an app module still running against a database schema it was never built for, or a schema migration that partially applied while a dependent app module was never redeployed to match it. Never assume the deployer's module ordering or dependency graph protects you from this; explicitly check which modules are actually running and what version/state they're in (`cf apps`, checking route mappings, comparing deployed module versions against the MTA's expected versions) rather than assuming the partial failure had no side effects. Once the root cause is identified and fixed (increased quota, corrected config), re-running `cf deploy` with the corrected MTA is the standard recovery path, but you should still verify the resulting state afterward rather than trusting the deployer to have self-corrected any mismatch from the earlier partial failure.",
    hindiExplanation:
      "Pehle, deployment logs se specific failure reason nikaalo — ek failed resource provisioning (jaise HANA quota exceeded), ek module jo start hone mein fail hua (bad config, missing env var), ya ek timeout — sab alag root causes hain jinhe alag fixes chahiye. Critically, MTA deployment process modules ke across atomicity guarantee nahi deta — poore MTA ko span karne wala koi true transactional rollback nahi hai, isliye ek partway failure genuinely landscape ko ek mixed-version state mein chhod sakta hai: ek app module jo abhi bhi ek database schema ke against chal raha hai jiske liye wo kabhi bana hi nahi tha, ya ek schema migration jo partially apply hui jabki ek dependent app module usse match karne ke liye kabhi redeploy nahi hua. Kabhi assume mat karo ki deployer ka module ordering ya dependency graph tumhe isse protect karta hai; explicitly check karo kaunse modules actually chal rahe hain aur kis version/state mein hain. Root cause identify aur fix hone ke baad (quota badhaya, config correct kiya), `cf deploy` ko corrected MTA ke saath dobara chalana standard recovery path hai, lekin tumhe fir bhi resulting state verify karna chahiye baad mein, deployer pe bharosa karne ki jagah ki usne khud hi mismatch correct kar diya hoga.",
    interviewExplanation:
      "I'd give the diagnosis-then-recovery sequence: 'First, check deployment logs for exactly which module/resource failed and why — quota, bad config, timeout are all different fixes. MTA deployment doesn't guarantee atomicity across modules, so I'd never assume it's fine — a partial failure can genuinely leave mismatched module versions running. I'd explicitly verify what's actually running and its version rather than assume. Once I fix the root cause, re-running cf deploy with the corrected MTA is the standard recovery, but I'd still verify the resulting state afterward rather than trust it self-corrected.'",
    diagramNote:
    ...
    interviewTip:
      "If asked 'is a failed MTA deployment dangerous', the accurate answer is yes, potentially — the deployer gives you no atomic/transactional rollback across modules, so a partial failure can genuinely leave mismatched module versions running; the safe answer always includes explicit post-failure verification, never an assumption of built-in protection.",
    followupQuestions: [
    ...
    commonMistakes: [
      "Assuming the MTA deployer provides atomic/transactional rollback across modules and that a partial failure can't leave a mismatched state.",
    ...
    importantPoints: [
      ...
      "MTA deployment provides no cross-module atomicity guarantee — a partial failure can genuinely leave mismatched module/schema versions running.",
      "Verify actual running state and version explicitly rather than assuming the deployer protected you.",
      "Fix the root cause, then re-run cf deploy, and verify the resulting state afterward rather than trusting it self-corrected.",
    ],
    revisionNotes: "Failed cf deploy: check logs for specific failure → MTA deployment has no cross-module atomicity guarantee, so explicitly verify actual running state/version (don't assume) → fix root cause → re-run deploy → verify resulting state again.",
  },
  {
    id: "scen-q6",
    topic: "CAP Crash",
  ...
  },
  {
    ...
    estimatedMinutes: 2,
    expectedAnswer:
      "Since users can still authenticate but now fail authorization, the issue is specific to scope/role changes in the xs-security.json update — check whether a scope was accidentally renamed or removed, which breaks the Role Template bound to that scope, which in turn breaks the Role(s) built from that template, cascading up to every Role Collection that includes those Roles — rather than a broader authentication/XSUAA health problem. Role Collections don't reference scopes directly; they reference Roles, so the break always propagates through that chain.",
    detailedAnswer:
      "The distinction between authentication and authorization (covered earlier) is the key diagnostic clue here: if users are authenticating successfully (getting a valid token) but then failing at the authorization check, the problem is specific to scopes/roles, not XSUAA's core authentication function. The actual XSUAA authorization chain is Role Collection → Role → Role Template → Scope — Role Collections never reference scopes directly, they reference Roles. So when an xs-security.json update accidentally renames, removes, or restructures a scope, what actually breaks first is the Role Template that was bound to that scope name; that break then propagates to every Role built from that template, and from there cascades up to every Role Collection that includes one of those Roles. The end symptom (existing users suddenly failing authorization) is the same, but the root cause sits at the Role Template's binding to the scope, not at the Role Collection level. The fix is comparing the old and new xs-security.json to identify exactly what scope name changed, then either reverting that specific change or updating the affected Role Template(s) — and, if necessary, re-verifying the Roles and Role Collections built on top of them — to reference the new scope name.",
    hindiExplanation:
      "Authentication aur authorization ke beech distinction (jo pehle cover hui) yahan key diagnostic clue hai: agar users successfully authenticate ho rahe hain (ek valid token pa rahe hain) lekin phir authorization check pe fail ho rahe hain, problem specifically scopes/roles mein hai, XSUAA ke core authentication function mein nahi. Actual XSUAA authorization chain hai Role Collection → Role → Role Template → Scope — Role Collections kabhi scopes ko directly reference nahi karte, wo Roles ko reference karte hain. Isliye jab ek xs-security.json update accidentally ek scope rename, remove, ya restructure kar deta hai, jo actually pehle break hota hai wo hai us scope se bound Role Template; wo break fir har Role tak propagate hota hai jo us template se bana hai, aur wahan se cascade hoke har Role Collection tak jaata hai jisme un Roles mein se koi shamil hai. End symptom (existing users achanak authorization fail karna) same hai, lekin root cause Role Collection level pe nahi, Role Template ke scope binding pe hota hai.",
    interviewExplanation:
      "I'd use the authentication-versus-authorization distinction as the key clue, then walk the actual chain: 'Since users still authenticate but fail authorization, this is scope/role-specific, not core XSUAA health. The real chain is Role Collection → Role → Role Template → Scope — Role Collections reference Roles, not scopes directly. So if an xs-security.json update renamed or removed a scope, what actually broke first is the Role Template bound to that scope, which broke the Roles built from it, which cascaded up to every Role Collection containing those Roles. I'd diff the old and new xs-security.json to find exactly what scope name changed, then either revert it or update the affected Role Template and re-verify the Roles/Role Collections built on top.'",
    diagramNote:
      "'Users authenticate fine, fail authorization' → 'scope/role-specific issue, not XSUAA health' → 'Diff old vs new xs-security.json' → 'Find renamed/removed scope' → 'Breaks the Role Template bound to that scope' → 'Breaks the Role(s) built from that template' → 'Cascades to every Role Collection containing those Roles → authz fails'.",
    diagramMermaid: `flowchart TD
    ...
    D --> E["Breaks the Role Template<br/>bound to that scope"]
    E --> F["Breaks the Role(s)<br/>built from that template"]
    F --> G["Cascades to every Role Collection<br/>containing those Roles → authz fails"]`,
    realProjectExample:
      "A well-intentioned xs-security.json cleanup renamed a scope from 'OrderRead' to 'Order.Read' for consistency. That broke the Role Template that was bound to the old 'OrderRead' scope name, which in turn broke the Role built from that template, which cascaded up to every Role Collection that included that Role — the actual chain was Role Collection → Role → Role Template → Scope, not Role Collections referencing the scope directly. Diffing the two versions of the file immediately revealed the rename, and the fix was updating the Role Template to bind to the new scope name, then re-verifying the downstream Roles and Role Collections.",
    interviewTip:
    ...
    importantPoints: [
      ...
      "The real chain is Role Collection → Role → Role Template → Scope — Role Collections reference Roles, not scopes directly.",
      "A likely cause is a scope renamed/removed in the xs-security.json update, which breaks the Role Template bound to it, cascading to the Roles and then the Role Collections built on top.",
      "Diff the old and new configuration to find the exact change, then fix by reverting or updating the Role Template's scope binding (and re-verifying downstream Roles/Role Collections).",
    ],
    revisionNotes: "Auth succeeds, authorization fails right after an xs-security.json deploy = likely a renamed/removed scope that breaks the Role Template bound to it, cascading through the Role(s) to the Role Collections that include them (chain: Role Collection → Role → Role Template → Scope) — diff old vs new config to find the exact change.",
  },
  {
    id: "scen-q23",
    topic: "Authentication Failure",
