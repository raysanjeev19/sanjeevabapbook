...
export const kymaRuntimeQuestions: BtpQuestion[] = [
  ...
  },
  {
    ...
    detailedAnswer:
      "Kubernetes RBAC has two levels: a Role (namespace-scoped) or ClusterRole (cluster-wide) defines a set of permitted verbs (get, list, create, delete) on specific resource types; a RoleBinding (namespace-scoped) or ClusterRoleBinding (cluster-wide) then grants that role to specific users/groups/service accounts. To restrict a team to only their namespace, you'd create a Role in that namespace with the permissions they need, then a RoleBinding in the same namespace linking their identity to that Role — critically using a RoleBinding and not a ClusterRoleBinding, since a ClusterRoleBinding always applies cluster-wide regardless of which Role or ClusterRole it references, which would defeat the namespace confinement.",
    hindiExplanation:
