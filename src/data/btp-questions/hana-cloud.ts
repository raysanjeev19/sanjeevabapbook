...
export const hanaCloudQuestions: BtpQuestion[] = [
  ...
  },
  {
    ...
    detailedAnswer:
      "Without input parameters, if a Calculation View's logic needs a specific value (say, converting all amounts to a chosen target currency, or filtering to a specific reporting period), that value would have to be hardcoded into the view itself, requiring a separate view — or manual editing — for every different value needed. An input parameter declared on the view lets the calling query supply that value dynamically (e.g. `CE_SALES(PLACEHOLDER.\"$$TARGET_CURRENCY$$\" => 'USD')`), which the view's internal calculation logic (a currency-conversion node, a filter condition) then references, making one reusable view serve many different runtime scenarios rather than needing a proliferation of near-duplicate views.",
    hindiExplanation:
