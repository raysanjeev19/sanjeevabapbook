...
export function HomeExperience() {
  ...
  return (
    <main className="min-h-screen bg-background pb-20">
      {/* === NAV === */}
      <nav className="mx-auto w-full max-w-6xl px-4 pt-5 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between rounded-xl border border-border bg-surface px-4 py-2.5">
          <Link href="/" className="flex items-center gap-2.5">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-[9px] font-medium uppercase tracking-[0.18em] text-faint">SAP ABAP Mastery</span>
            </div>
          </Link>
      ...
      <section className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        <div className="grid items-center gap-10 py-12 lg:grid-cols-[1.1fr_0.9fr] lg:gap-16 lg:py-16">
          <div className="animate-fade-in">
            <p className="text-sm font-medium text-accent">{getGreeting()}</p>

            <h1 className="mt-4 font-serif text-[2.6rem] font-semibold leading-[1.1] tracking-tight text-foreground sm:text-5xl lg:text-[3.5rem]">
              Your SAP ABAP
              <br />
              interview, read like a book.
            </h1>
            ...
            <div className="mt-8 flex flex-col gap-2.5 sm:flex-row">
              <Button asChild>
                <Link href={`/questions/${continueQuestion}`}>
                  <BookOpen size={16} /> {recent.length > 0 ? "Continue reading" : "Start reading"}
                </Link>
              </Button>
            ...
            <div className="mt-9 flex items-center gap-6 text-xs text-faint">
              <span className="flex items-center gap-1.5"><BookMarked size={13} /> {chapters.length} chapters</span>
              <span className="flex items-center gap-1.5"><Mic size={13} /> Voice AI</span>
              <span className="flex items-center gap-1.5"><GraduationCap size={13} /> Hinglish</span>
            </div>
          </div>
            ...
            return (
              <Link key={chapter.slug} href={`/chapters/${chapter.slug}`}>
                <Card
                  ...
                  className="group relative h-full overflow-hidden p-4 hover:border-border-strong hover:bg-surface-2"
                >
                  {prog > 0 && (
                    <div className="absolute bottom-0 left-0 right-0 h-[2px] bg-border">
                      <div className="h-full transition-[width]" style={{ width: `${prog}%`, backgroundColor: chapter.color }} />
                    </div>
                  )}
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center justify-between gap-1.5">
                        <h3 className="truncate text-sm font-semibold text-foreground">{chapter.title}</h3>
                        {prog > 0 && (
                          <span className="shrink-0 text-[10px] font-bold text-accent">{prog}%</span>
                        )}
                      </div>
